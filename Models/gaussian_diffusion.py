...
from Models.interpretable_diffusion.seed import Seed
...
class Diffusion_TS(nn.Module):
    def __init__(
        ...
        print('n_layer_enc, n_layer_dec = ', n_layer_enc, n_layer_dec)
        if pred_len is None:
            pred_len = seq_len
        ...
        if net in 'Transformer':
            print('model = Transformer', net)
            self.model = Transformer(n_feat=n_node, n_channel=seq_len, n_layer_enc=n_layer_enc, n_layer_dec=n_layer_dec,
                                 n_heads=n_heads, attn_pdrop=attn_pd, resid_pdrop=resid_pd, mlp_hidden_times=mlp_hidden_times,
                                 max_len=seq_len, n_embd=d_model, conv_params=[kernel_size, padding_size], **kwargs)
        elif net in 'EDS4':
            print('model = EDS4', net)
            self.model = Seed(n_node=n_node, seq_len=seq_len, pred_len=pred_len, n_layer_enc=n_layer_enc, 
                              n_layer_dec=n_layer_dec, n_embd=d_model, dropout=resid_pd, 
                              conv_params=[kernel_size, padding_size], **kwargs)
        else:
            raise TypeError('No Model Selected')
    ...
    def generate_mts(self, batch_size=16):
        n_node, seq_length = self.n_node, self.seq_length
        sample_fn = self.fast_sample if self.fast_sampling else self.sample
        return sample_fn((batch_size, seq_length, n_node))
    ...
    def _train_loss(self, x_start, t, target=None, noise=None, padding_masks=None):
        #noise = default(noise, lambda: torch.randn_like(x_start))
        noise = default(noise, lambda: torch.ones_like(x_start))
        if target is None: ### generation
            target = x_start
  
        x = x_start # self.q_sample(x_start=x_start, t=t, noise=noise)  # noise sample
    ...
    def forward(self, x, **kwargs):
        b, c, n, device, n_node, = *x.shape, x.device, self.n_node
        assert n == n_node, f'number of variable must be {n_node}'
    ...
    def predict(self, x):
        ...
        #x = self.q_sample(x_start=x, t=t, noise=noise)  # noise sample
        model_out = self.output(noise, t)
        return model_out
    ...
    def fast_sample_infill(self, x, target, sampling_timesteps, partial_mask=None, clip_denoised=True, mode=None, model_kwargs=None):
        shape = x.shape
        ...
        times = list(reversed(times.int().tolist()))
        time_pairs = list(zip(times[:-1], times[1:]))  # [(T-1, T-2), (T-2, T-3), ..., (1, 0), (0, -1)]
        img = x if mode in 'predict' else torch.randn(shape, device=device)
