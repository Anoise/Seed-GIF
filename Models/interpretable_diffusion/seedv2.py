...
from Models.interpretable_diffusion.s4 import S4
...
class S4Decoder(nn.Module):
    def __init__(self, n_embd, n_feat, pred_len, dropout=0.1):
        super().__init__()
        ...
        self.trend = TrendBlock(n_feat, pred_len, n_embd)
...
class Decoder(nn.Module):
    def __init__(self, n_layer, n_embd, n_feat, pred_len, dropout=0.1):
        super().__init__()
        self.blocks = nn.Sequential(*[
            S4Decoder(n_embd=n_embd, n_feat=n_feat, pred_len=pred_len, dropout=dropout) 
    ...
    def forward(self, x, t, x_enc, padding_masks=None, label_emb=None):
        ...
        trend = torch.zeros((b, c, self.pred_len), device=x.device)
        for block_idx in range(len(self.blocks)):
...
class Seed(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.emb = Conv_MLP(seq_len, n_embd, resid_pdrop=dropout)
        self.inverse = Conv_MLP(n_embd, pred_len, resid_pdrop=dropout)
        ...
        self.combine_s = nn.Conv1d(n_embd, pred_len, kernel_size=kernel_size, stride=1, padding=padding,
                                   padding_mode='circular', bias=False)
        self.combine_m = nn.Conv1d(n_layer_dec, 1, kernel_size=1, stride=1, padding=0,
                                   padding_mode='circular', bias=False)

        self.encoder = Encoder(n_layer=n_layer_enc,n_embd=n_embd, n_feat=n_node, dropout=dropout)
        self.pos_enc = LearnablePositionalEncoding(n_embd, dropout=dropout, max_len=n_node)

        self.decoder = Decoder(n_layer=n_layer_dec,n_embd=n_embd, n_feat=n_node, pred_len=pred_len, dropout=dropout)
        self.pos_dec = LearnablePositionalEncoding(n_embd, dropout=dropout, max_len=n_node)
    ...
    def forward(self, input, t, padding_masks=None, return_res=False):
        ...
        season_error = self.combine_s(season) + res - res_m
        trend = self.combine_m(mean) + res_m + trend

        if return_res:
            return trend, self.combine_s(season.transpose(1, 2)).transpose(1, 2), res - res_m

        return trend.transpose(1,-1), season_error.transpose(1,-1)
