...
def align_shape(x1, x2, dim=1):
    shape = list(x1.shape)
    diff_len = x1.shape[dim] - x2.shape[dim]
    if diff_len < 0:
        shape[dim] = -diff_len
        zeros = torch.zeros(shape).to(x1.device)
        x1 = torch.cat((x1, zeros), dim)
    elif diff_len > 0:
        shape[dim] = diff_len
        zeros = torch.zeros(shape).to(x2.device)
        x2 = torch.cat((x2, zeros), dim)
    return x1, x2
        
        
def algin_to_x1(x1, x2, dim=1):
    shape = list(x1.shape)
    diff_len = x1.shape[dim] - x2.shape[dim]
    if diff_len < 0:
        x2 = x2[:,:-diff_len,:]
