...
#     --model_id $model_id_name'_MSE_'$seq_len'_'$pred_len \
#     --model $model_name \
#     --data $data_name \
#     --seq_len $seq_len \
#     --pred_len $pred_len \
...
#     >logs/$data_name'_'LongForecasting/$model_name'__MSE_'$model_id_name'_'$seq_len'_'$pred_len.log
# done

model_name=DecomLinearV2
echo "------------- model --------------" $model_name
for pred_len in 72 #24 36 48 72
do
    echo "------------- pre-len --------------" $pred_len
    python -u run.py \
    --random_seed $random_seed \
